...
public class Main : SR2EExpansionV1
{
    ...
    public static IEnumerator OnSaveLoaded()
    {
        if (ClientActive() && !ServerActive())
        {
            ...
            sceneContext.PlayerState._model._currencies[1].Amount = save.newbucks;
            sceneContext.PlayerState._model.SetCurrency(GameContext.Instance.LookupDirector._currencyList[1].Cast<ICurrency>(), save.rainbowCoins);
                ...
                switch (plot.type)
                {
                    case Il2Cpp.LandPlot.Id.GARDEN:
                        ...
                        model.resourceGrowerDefinition =
                            gameContext.AutoSaveDirector._saveReferenceTranslation._resourceGrowerTranslation.RawLookupDictionary._entries.FirstOrDefault(x =>
                                x.value._primaryResourceType == identifiableTypes[plot.cropIdent]).value;
