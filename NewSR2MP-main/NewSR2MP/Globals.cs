...
namespace NewSR2MP
{
    public static class Globals
    {
        ...
        {
            return gameContext.AutoSaveDirector._saveReferenceTranslation.GetPersistenceId(ident);
        }
        ...
        public static void CreateWeatherLookup(SaveReferenceTranslation translation)
        {
            var states = new Dictionary<int, WeatherStateDefinition>();
            var states2 = new Dictionary<string, int>();
            int i = 0;
            foreach (var state in translation._weatherStateTranslation.RawLookupDictionary)
            {
            ...
            SceneGroup sceneGroup)
        {
            ...
            if (model != null)
            {
                ...
                var data = CreateActorDataFromModel( // 1.0.0 figure out what to do with this
                    model,
                    gameContext.AutoSaveDirector._saveReferenceTranslation);
        ...
        public static void DeregisterActor(ActorId id)
        {
            ...
            //foreach (var actor in gameContext.AutoSaveDirector..Actors)
            //{
            ...
            //if (found)
            //    gameContext.AutoSaveDirector...gameState.Actors.RemoveAt(idx);
        }
        ...
        public static ActorDataV02 CreateActorDataFromModel(
            IdentifiableModel model,
            SaveReferenceTranslation saveReferenceTranslation)
        {
            var actorModel = model.TryCast<ActorModel>();
            var gadgetModel = model.TryCast<GadgetModel>();
            
            var statusEffectToPersistenceId = saveReferenceTranslation._statusEffectTranslation;
            ActorDataV02 actorDataV = new ActorDataV02();

            IdentifiableType ident = model.ident;
            actorDataV.TypeId = saveReferenceTranslation.GetPersistenceId(ident);
            ...
            if (actorModel != null)
            {
                foreach (var effect in actorModel.statusEffects)
                {
                    var effectV01 = new StatusEffectV01()
                    {
                        ...
                        ID = saveReferenceTranslation.GetPersistenceId(effect.key)
                    };
            ...
            {
                model.TryCast<SlimeModel>().Pull(actorDataV, gameContext.AutoSaveDirector._saveReferenceTranslation.Cast<ISaveReferenceTranslation>());
            }
            else if (model.TryCast<AnimalModel>() != null)
            {
                model.TryCast<AnimalModel>().Pull(actorDataV, gameContext.AutoSaveDirector._saveReferenceTranslation.Cast<ISaveReferenceTranslation>());
            }
        ...
        public static AmmoSlot[] AmmoDataToSlotsSRMP(List<NetworkAmmoDataV01> ammo)
        {
            ...
            for (int i = 0; i < ammo.Count; i++)
            {
                ...
                {
                    Count = ammo[i].count,
                    _id = identifiableTypes[ammo[i].ident],
                    Emotions = new float4(ammo[i].emotionX, ammo[i].emotionY, ammo[i].emotionZ, ammo[i].emotionW)
                };;
            }
        ...
        public static List<NetworkAmmoDataV01> SlotsToSRMPAmmoData(AmmoSlot[] slots)
        {
            ...
            foreach (var slot in slots)
            {
                ...
                {
                    count = slot.Count,
                    ident = GetIdentID(slot._id),
                    
                    emotionX = slot.Emotions.x,
                    emotionY = slot.Emotions.y,
                    emotionZ = slot.Emotions.z,
                    emotionW = slot.Emotions.w,
                });
            }
        ...
        public static int GetSlotIDX(this AmmoSlotManager ammo, IdentifiableType id)
        {
            ...
            for (int j = 0; j < ammo._ammoModel.Slots.Count; j++)
            {
                ...
                if (!isSlotNull)
                    isSlotFull = ammo.Slots[j].Count >= ammo.GetSlotMaxCount(j);
                else
                    isSlotFull = false;
        ...
        {
            var newAmmo = new AmmoSlotManager(sceneContext.PlayerState._ammoSlotDefinitions);
            newAmmo._ammoModel = new AmmoModel(sceneContext.PlayerState.Ammo._ammoModel._unitPropertyTracker);
            return newAmmo;
        }
